...
namespace LAB5
{
    class LAB5
    {
        ...
        static TimeSpan longestIdleTime = TimeSpan.Zero; // для отображения периодабездействия 
        ...
        static void Main()
        {
            while (true)
            {
                ...
                //предложенные действия в соотвествии с условием задания
                Console.Write("\nДЕЙСТВИЕ: ");
                if (!int.TryParse(Console.ReadLine(), out int choice) || choice < 1 || choice > 7) //тут проверяем, введено ли число от 1 до 7
                {
